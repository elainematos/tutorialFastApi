...
def test_lista_tarefas_formato_jason():
    client = TestClient(app)
    response = client.get("/tarefas")
...
#def test_tarefa_deve_ter_estado():
   # client = TestClient(app)
   # response = client.post("/tarefas", json={"titulo": "Addams", "descricao": "A123456"})
   # assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
   #TAREFAS.clear()
...
def test_quando_criar_uma_tarefa_seu_id_deve_ser_unico():
    ...
    tarefa1 = {"titulo": "titulo1", "descricao": "descricao1", "estado": "Finalizado"}
    tarefa2 = {"titulo": "titulo2", "descricao": "descricao1", "estado": "Finalizado"}
...
def test_tarefa_criada_estado_padrao_naoFinalizado():
    ...
    tarefa = {"titulo": "Nancy Drew", "descricao":"A121223"}
    response = client.post("/tarefas", json=tarefa)
    assert response.json()["estado"] == "Não Finalizado" 
    TAREFAS.clear()
